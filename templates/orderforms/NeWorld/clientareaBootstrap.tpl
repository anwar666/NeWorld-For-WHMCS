<script type="text/javascript" src="modules/servers/solusvmpro/js/get_user_data.js?v65"></script>
<script type="text/javascript" src="modules/servers/solusvmpro/js/rebuild.js?v65"></script>
...
<link rel="stylesheet" href="modules/servers/solusvmpro/templates/assets/css/style.css?v65">
<link rel="stylesheet" href="modules/servers/solusvmpro/templates/assets/css/sweetalert.css?v65">
...
<div class="row">
	...
	<div class="col-md-12">
		<div class="meta-main">
			...
			<a href="index.php?m=ReNew&amp;id={$id}" class="btn btn-sm btn-info"><i class="md md-account-balance-wallet"></i> 提前续费</a>
				<a href="javascript:location.reload()" class="btn btn-sm btn-primary"><i class="fa fa-refresh"></i> {$LANG.solusvmpro_refresh}</a>
				{if $showcancelbutton || $packagesupgrade}
				{if $packagesupgrade}
                	<a href="upgrade.php?type=package&amp;id={$id}" class="btn btn-sm btn-success"><i class="fa fa-arrow-up" aria-hidden="true"></i>{$LANG.upgrade}</a>
                {/if}
				<a href="clientarea.php?action=cancel&amp;id={$id}" class="btn btn-sm btn-danger {if $pendingcancellation}disabled{/if}"><i class="md md-delete"></i> {if $pendingcancellation}{$LANG.cancellationrequested}{else}{$LANG.clientareacancelrequestbutton}{/if}</a>
				{/if}
			</div>
		</div>
	</div>
...
<div class="row solusvm" style="display: none">
	...
	<div class="col-md-12 detail" style="display: none">
		<div class="panel panel-default panel-amount">
			<div class="panel-heading">
            	<h3 class="panel-title"><i class="fa fa-star"></i>&nbsp;{$LANG.clientareaproductdetails}</h3>
        	</div>
	        	...
	        	<div class="row">
		        	...
		        	<div class="col-md-7">
			        	<div class="left-main">
				        	<ul class="list-unstyled" style="margin-bottom: 0;">
				        		...
				        		<li>{$LANG.solusvmpro_ipAddress}：<span>{$info['mainip']} {if $info['ipcsv'] != $info['mainip']}<i class="icons icon-settings" style="vertical-align: middle;height: 16px;width: 15px;font-size: 15px;" data-container="body" data-toggle="popover" data-placement="bottom" data-content="{$info['ipcsv']}"></i>{/if}</span></li>
		        	...
		        	<div class="col-md-5">
			        	<div class="right-main">
				        	{if ($info['type'] == 'openvz')}
				        	<h5>{$LANG.solusvmpro_memory}:
					        		<span class="pull-right">{$info['memoryused']} / {$info['memorytotal']} <i>({$info['memoryfree']} {$LANG.solusvmpro_free})</i></span></h5>
				        	...
				        	{if $info['type'] == 'openvz' || $info['type'] == 'xen'}
				        		<h5>{$LANG.solusvmpro_disk}:
					        		<span class="pull-right">{$info['hddused']} / {$info['hddtotal']} <i>({$info['hddfree']} {$LANG.solusvmpro_free})</i></span></h5>
								<div class="progress active progress-lg">
				        	...
				        	{/if}
				        	<h5>{$LANG.solusvmpro_bandwidth}:
					        	<span class="pull-right">{$info['bandwidthused']} / {$info['bandwidthtotal']} <i>({$info['bandwidthfree']} {$LANG.solusvmpro_free})</i></span></h5>
							<div class="progress active progress-lg">
			        	...
			        	<div class="btm-main">
				        	<ul class="list-unstyled row">
					        	...
					        	<li class="col-sm-4 col-md-2">
					        		<a href="javascript:if(confirm('您确定要{$LANG.solusvmpro_reboot}吗？'))location='clientarea.php?action=productdetails&id={$serviceid}&serveraction=custom&a=reboot';" class="btn btn-default"><i class="fa fa-repeat text-primary"></i>{$LANG.solusvmpro_reboot}</a>
					        	</li>
					        	{/if}
					        	{if $info['displayboot'] == 1}
					        	<li class="col-sm-4 col-md-2">
					        		<a href="javascript:if(confirm('您确定要{$LANG.solusvmpro_boot}吗？'))location='clientarea.php?action=productdetails&id={$serviceid}&serveraction=custom&a=boot';" class="btn btn-default"><i class="fa fa-play text-success"></i>{$LANG.solusvmpro_boot}</a>
					        	</li>
					        	{/if}
					        	{if $info['displayshutdown'] == 1}
					        	<li class="col-sm-4 col-md-2">
					        		<a href="javascript:if(confirm('您确定要{$LANG.solusvmpro_shutdown}吗？'))location='clientarea.php?action=productdetails&id={$serviceid}&serveraction=custom&a=shutdown';" class="btn btn-default"><i class="fa fa-stop text-danger"></i>{$LANG.solusvmpro_shutdown}</a>
					        	</li>
					        	{/if}
					        	...
					        	<li class="col-sm-4 col-md-2">
					        		<a href="#" onClick="window.open('modules/servers/solusvmpro/console.php?id={$serviceid}','_blank','width=670,height=400,status=no,location=no,toolbar=no,menubar=no')" class="btn btn-default"><i class="fa fa-terminal"></i>{$LANG.solusvmpro_serialConsole}</a>
					        	</li>
					        	{/if}
					        	{if $info['displayhtml5console'] == 1}
					        	<li class="col-sm-4 col-md-2">
					        		<a href="#" onClick="window.open('modules/servers/solusvmpro/html5console.php?id={$serviceid}','_blank','width=870,height=600,status=no,resizable=yes,copyhistory=no,location=no,toolbar=no,menubar=no,scrollbars=1')" class="btn btn-default"><i class="fa fa-terminal"></i>{$LANG.solusvmpro_html5Console}</a>
					        	</li>
					        	{/if}
					        	{if $info['displayvnc'] == 1}
					        	<li class="col-sm-4 col-md-2">
					        		<a href="#" onClick="window.open('modules/servers/solusvmpro/vnc.php?id={$serviceid}','_blank','width=400,height=200,status=no,location=no,toolbar=no,menubar=no')" class="btn btn-default"><i class="fa fa-terminal"></i>{$LANG.solusvmpro_vnc}</a>
					        	</li>
					        	{/if}
					        	{if $info['displaypanelbutton'] == 1}
					        	<li class="col-sm-4 col-md-2">
					        		<a href="#" id="controlpanellink" class="btn btn-default"><i class="fa fa-terminal"></i>{$LANG.solusvmpro_controlPanel}</a>
					        	</li>
					        	{/if}
	...
	<div class="col-md-12">
		<div class="panel panel-default panel-amount">
			<div class="panel-heading">
            	<h3 class="panel-title"><i class="fa fa-star"></i>&nbsp;{$LANG.solusvmpro_paymentDetails}</h3>
        	</div>
        	<div class="panel-body">
		...
		<div class="rebuild-content">
			<div class="row" id="templates">
				...
				<div class="{if {$key|count} <= 3}col-sm-3{else}col-sm-4{/if}">
					<div class="template">
						<i class="os-icon os-icon-lg os-icon-os os-icon-{$key}"></i>
						<strong class="systemname">{$key}</strong>
						<span>{$LANG.Selectsystem}</span>
						<ul class="list-unstyled system">
						{foreach from=$value item=os}
							<li data-os="{$key}" data-template="{$os['filename']}">{$os['name']}</li>
						{/foreach}
						</ul>
					</div>
				</div>
		    ...
		    <div class="text-center" id="loading" style="display: none">
			      <i class="text-center fa fa-spinner fa-pulse fa-5x fa-fw margin-bottom" style="margin: 50px;"></i>
			</div>
		</div>
		
	    <div class="rebuild-footer">
		    <span class="pull-left" id="systemname"></span>
		    <button type="submit" class="confirm">{$LANG.orderForm.yes}</button>
	    </div>
	</form>
    <div class="text-center rootpass_loading" style="display: none">
	      <i class="text-center fa fa-spinner fa-pulse fa-5x fa-fw margin-bottom" style="margin: 20px;color: #FFF;"></i>
	</div>
	<code>
